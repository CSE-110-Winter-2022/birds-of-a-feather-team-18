...
public class SessionListTest {
    ...
    public void setupDb() {
        ...
        // Mock adding Favorites Session from NameLogin
        Session favoritesSession = new Session("favoritesSession", "Favorites");
        testDB.sessionsDao().insert(favoritesSession);
        ...
        Session newSession = new Session(UUID.randomUUID().toString(), "testSession");
    ...
    @Test
    public void test_sessionList() {
        ActivityScenario<SessionListActivity> scenario = ActivityScenario.launch(SessionListActivity.class);
        scenario.moveToState(Lifecycle.State.CREATED);

        scenario.onActivity(activity -> {

            // Two Sessions, Favorites Session + testSession
            List<Session> sessionTestList = activity.sessionsViewAdapter.getSessions();

            assertEquals(2, sessionTestList.size());

            // Favorites is first Session
            assertEquals("favoritesSession", sessionTestList.get(0).sessionId);
        });
    }
}